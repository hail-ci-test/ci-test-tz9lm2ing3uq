...
class BuildImage2Step(Step):
    def __init__(
        ...
        new_docker_prefix = get_global_config().get('new_docker_prefix', None)
        if new_docker_prefix is not None:
            self.push_targets.append(f'{new_docker_prefix}{self.image[len(DOCKER_PREFIX):]}')
